...
\def\fl@post{}
\csname newif\expandafter\endcsname\csname if@fl@found\endcsname
%\newif\if@fl@found
...
\def\fakenumfont#1{#1}
\def\fl@item{%
  % fakenum always prints its own fake label, regardless of any
  % surrounding list. It is paragraph text, not a list item.
  %
  % We deliberately do NOT call \refstepcounter{fakenum}, because that
  % would register a hyperref anchor named "fakenum.N" -- and since the
  % counter resets on every fakenum environment, the second instance in
  % the document collides with the first. Instead we step the counter
  % manually and install our own \@currentHref built from a per-instance
  % serial number, so every anchor is unique across the document.
  \stepcounter{fakenum}%
  \protected@edef\@currentlabel{\fl@pre\fl@fmt\fl@post}%
  \edef\@currentHref{fakenum.\the\c@fakenum@inst.\the\c@fakenum}%
...
\newenvironment{fakenum}[1][(i)]{%
  ...
  \let\item\fl@item
  % 5. Patch \exi so that \sn (= \exi{}) is a no-op inside fakenum.
  %    Non-empty \exi{label} is passed through unchanged (in case the
  %    user genuinely wants gb4e's label behaviour here).
  \let\fl@savedexi\exi
  \def\exi##1{%
    \def\fl@tmp{##1}%
    \ifx\fl@tmp\@empty
      % swallow optional [foo] that \sn may produce, then do nothing
      \@ifnextchar[{\@gobble}{}%
    \else
      \fl@savedexi{##1}%
    \fi
